        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold mb-2">
              Welcome to Scholastic Desktop! 👋
            </h1>
            <p className="text-indigo-100">
              Here's what's happening with your Scholastic platform today.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div variants={itemVariants} className="bg-white rounded-xl shadow-sm border border-gray-200">
          ...
          <div className="p-6 space-y-3">
            {[
              { name: 'Add New User', icon: '👤', color: 'bg-blue-50 text-blue-600' },
              { name: 'Create Institution', icon: '🏢', color: 'bg-green-50 text-green-600' },
              { name: 'Manage Roles', icon: '🔐', color: 'bg-purple-50 text-purple-600' },
              { name: 'View Reports', icon: '📊', color: 'bg-orange-50 text-orange-600' },
            ].map((action, index) => (
              <motion.button
                key={index}
