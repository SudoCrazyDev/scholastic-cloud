...
class AiPlannerController extends Controller
{
    ...
    public function generateTopics(Request $request, string $subjectId, string $quarter): JsonResponse
    {
        ...
        $templates = [
            "Introduction to %s",
            "Core Concepts of %s",
            "Key Terms and Vocabulary in %s",
            "Fundamentals and Skills in %s",
            "Applied Practice: %s",
            "Problem Solving in %s",
            "Real-world Applications of %s",
            "Collaboration and Activities: %s",
            "Review and Reinforcement: %s",
            "Performance Task: %s",
            "Summative Review: %s",
        ];
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $title = sprintf($tpl, $subjectTitle);
            if ($i >= count($templates)) {
                $title .= ' (Part ' . (int)(floor($i / count($templates)) + 1) . ')';
            }
            $suggestions[] = [
                'title' => $title,
                'description' => "Quarter {$quarterValidated['quarter']} topic for {$subjectTitle}.",
