...
/**
 * A subject teacher may issue a new portal password, and nothing more.
 *
 * The interesting cases are all about the edge of that grant: the teacher holds
 * `students` read-only, so the tests check they can reset a password while still
 * being unable to point the login at another email — which would otherwise be a
 * route to signing in as the student — or mint one for a student who has none.
 *
...
class StudentPortalPasswordResetTest extends TestCase
{
    ...
    public function test_a_student_without_a_login_is_refused_rather_than_given_one(): void
    {
