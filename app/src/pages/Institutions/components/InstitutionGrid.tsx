import React from 'react'
import { motion } from 'framer-motion'
import { 
  PencilIcon, 
  TrashIcon, 
  BuildingOfficeIcon,
  MapPinIcon,
  IdentificationIcon,
  GlobeAltIcon
} from '@heroicons/react/24/outline'
import { Checkbox } from '../../../components/checkbox'
import type { Institution } from '../../../types'

interface InstitutionGridProps {
  institutions: Institution[]
  loading: boolean
  error: string | null
  selectedRows: Institution[]
  onSelectionChange: (institutions: Institution[]) => void
  onEdit: (institution: Institution) => void
  onDelete: (institution: Institution) => void
}

export const InstitutionGrid: React.FC<InstitutionGridProps> = ({
  institutions,
  loading,
  error,
  selectedRows,
  onSelectionChange,
  onEdit,
  onDelete,
}) => {
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      onSelectionChange(institutions)
    } else {
      onSelectionChange([])
    }
  }

  const handleSelectInstitution = (institution: Institution, checked: boolean) => {
    if (checked) {
      onSelectionChange([...selectedRows, institution])
    } else {
      onSelectionChange(selectedRows.filter(item => item.id !== institution.id))
    }
  }

  const isSelected = (institution: Institution) => {
    return selectedRows.some(item => item.id === institution.id)
  }

  if (loading) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-center py-12">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-600"></div>
          <span className="ml-3 text-gray-600">Loading institutions...</span>
        </div>
      </div>
    )
  }

  if (error) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="text-center py-12">
          <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4">
            <svg className="w-8 h-8 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
            </svg>
          </div>
          <h3 className="text-lg font-medium text-gray-900 mb-2">Error Loading Institutions</h3>
          <p className="text-gray-500">{error}</p>
        </div>
      </div>
    )
  }

  if (institutions.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="text-center py-12">
          <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4">
            <BuildingOfficeIcon className="w-8 h-8 text-gray-400" />
          </div>
          <h3 className="text-lg font-medium text-gray-900 mb-2">No Institutions Found</h3>
          <p className="text-gray-500 mb-6">Get started by creating your first institution.</p>
        </div>
      </div>
    )
  }

  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      {/* Header with select all */}
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          <Checkbox
            checked={selectedRows.length === institutions.length && institutions.length > 0}
            onChange={handleSelectAll}
            indeterminate={selectedRows.length > 0 && selectedRows.length < institutions.length}
          />
          <span className="text-sm text-gray-600">
            {selectedRows.length > 0 
              ? `${selectedRows.length} of ${institutions.length} selected`
              : `${institutions.length} institutions`
            }
          </span>
        </div>
      </div>

      {/* Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {institutions.map((institution) => (
          <motion.div
            key={institution.id}
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.3 }}
            className={`relative group border rounded-lg p-4 transition-all duration-200 hover:shadow-md ${
              isSelected(institution) 
                ? 'border-indigo-500 bg-indigo-50' 
                : 'border-gray-200 bg-white hover:border-gray-300'
            }`}
          >
            {/* Selection checkbox */}
            <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
              <Checkbox
                checked={isSelected(institution)}
                onChange={(checked) => handleSelectInstitution(institution, checked)}
              />
            </div>

            {/* Logo placeholder */}
            <div className="w-12 h-12 bg-indigo-100 rounded-lg flex items-center justify-center mb-3">
              {institution.logo ? (
                <img 
                  src={institution.logo} 
                  alt={institution.title} 
                  className="w-8 h-8 rounded object-cover"
                  onError={(e) => {
                    // Hide the image and show placeholder on error
                    e.currentTarget.style.display = 'none'
                    e.currentTarget.nextElementSibling?.classList.remove('hidden')
                  }}
                />
              ) : null}
              <BuildingOfficeIcon className={`w-6 h-6 text-indigo-600 ${institution.logo ? 'hidden' : ''}`} />
            </div>

            {/* Institution info */}
            <div className="space-y-2">
              <div>
                <h3 className="font-semibold text-gray-900 text-sm truncate" title={institution.title}>
                  {institution.title}
                </h3>
                <p className="text-xs text-gray-500 font-mono">{institution.abbr}</p>
              </div>

              {/* Details */}
              <div className="space-y-1">
                {institution.address && (
                  <div className="flex items-center text-xs text-gray-600">
                    <MapPinIcon className="w-3 h-3 mr-1 flex-shrink-0" />
                    <span className="truncate" title={institution.address}>{institution.address}</span>
                  </div>
                )}
                
                {institution.division && (
                  <div className="flex items-center text-xs text-gray-600">
                    <GlobeAltIcon className="w-3 h-3 mr-1 flex-shrink-0" />
                    <span className="truncate">{institution.division}</span>
                  </div>
                )}

                {institution.region && (
                  <div className="flex items-center text-xs text-gray-600">
                    <GlobeAltIcon className="w-3 h-3 mr-1 flex-shrink-0" />
                    <span className="truncate">{institution.region}</span>
                  </div>
                )}

                {institution.gov_id && (
                  <div className="flex items-center text-xs text-gray-600">
                    <IdentificationIcon className="w-3 h-3 mr-1 flex-shrink-0" />
                    <span className="truncate font-mono">{institution.gov_id}</span>
                  </div>
                )}
              </div>
            </div>

            {/* Actions */}
            <div className="absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity flex space-x-1">
              <button
                onClick={() => onEdit(institution)}
                className="p-1 text-gray-400 hover:text-indigo-600 transition-colors"
                title="Edit institution"
              >
                <PencilIcon className="w-4 h-4" />
              </button>
              <button
                onClick={() => onDelete(institution)}
                className="p-1 text-gray-400 hover:text-red-600 transition-colors"
                title="Delete institution"
              >
                <TrashIcon className="w-4 h-4" />
              </button>
            </div>
          </motion.div>
        ))}
      </div>
    </div>
  )
} 