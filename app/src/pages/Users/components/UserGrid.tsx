...
export const UserGrid: React.FC<UserGridProps> = ({
  ...
  const handleSelectUser = (user: User, checked: boolean) => {
    if (checked) {
      ...
      onSelectionChange(selectedRows.filter(item => item.id !== user.id))
    }
  }

  const isSelected = (user: User) => {
    return selectedRows.some(item => item.id === user.id)
  }

  const getFullName = (user: User) => {
    const parts = [user.first_name, user.middle_name, user.last_name].filter(Boolean)
    const fullName = parts.join(' ')
    return user.ext_name ? `${fullName} ${user.ext_name}` : fullName
  }
        ...
        <div className="flex items-center space-x-3">
          ...
          <span className="text-sm text-gray-600">
            {selectedRows.length > 0 
              ? `${selectedRows.length} of ${users.length} selected`
              : `${users.length} users`
            }
          </span>
        </div>
          ...
          >
            ...
            <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
              <Checkbox
                ...
                onChange={(checked) => handleSelectUser(user, checked)}
              />
            </div>
            ...
            <div className="space-y-2">
              <div>
                <h3 className="font-semibold text-gray-900 text-sm truncate" title={getFullName(user)}>
                  {getFullName(user)}
                </h3>
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center text-xs text-gray-600">
                  ...
                  <span className="truncate">Born {formatDate(user.birthdate)}</span>
                </div>
                ...
                {user.is_new && (
                  <div className="flex items-center text-xs text-blue-600">
                    <span className="px-1.5 py-0.5 bg-blue-100 rounded text-xs">New User</span>
                  </div>
                )}
              </div>
            </div>
